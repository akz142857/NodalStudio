...
1. **顶栏 mode 是否收敛**为 `Database / Query`（右侧 History 分段接管对象级历史，顶栏保留全局时间线的话就不收敛）。
2. **激活态是否用主色绿** —— 绿色目前还表示「新增的表」，大面积使用可能稀释该信号（3.1）。
3. **顶栏 `.mode-switcher` 是否一并抽成共用 `.segmented`**，还是右栏单独实现。
4. Git / Cloud 移入 Settings 是否接受（2.4）。
5. 全局的分组/视图管理放左栏底部还是只在无选中时的 Semantics 分段里（3.2 / 3.3）。
6. 左栏是否需要跨连接同时展开。
